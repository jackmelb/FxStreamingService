...
import com.jz.fx.streaming.service.core.mock.MockPriceUpdateListener;
...
public class FxStreamingServiceCoreApplication implements CommandLineRunner {

	private static final Logger LOG = LoggerFactory.getLogger(FxStreamingServiceCoreApplication.class);

	ExecutorService listenerExecutor = Executors.newFixedThreadPool(4);
	...
	@Autowired
	@Qualifier("priceUpdateListener")
	private Listener<String> priceUpdateListener;

	@Autowired
	@Qualifier("priceUpdateHandler")
	private Runnable priceUpdateHandler;

	@Value("#{'${trading.ccys}'.split(',')}")
	private List<String> tradingCcys;

	@Autowired
	MockPriceUpdateListener mockPriceUpdateListener;
	...
	private static List<String> tradeableCcys = new ArrayList<>();
	...
	// private  BlockingQueue<UpdateInfo> priceUpdateQueue = new ArrayBlockingQueue<UpdateInfo>(100);
	...
	public void run(String... args) throws Exception {
		...
		FxStreamingServiceCoreApplication app = (FxStreamingServiceCoreApplication) applicationContext.getBean("application");
	...
	public boolean run(final String user, final String password, final String apiKey) {
		try {
			...
			//  Thread.sleep(500000000); // to let the streaming run a while
	...
	public static void main(String args[]) {
		SpringApplication.run(FxStreamingServiceCoreApplication.class, args);
	}
		...
		for (WatchlistsItem watchlist : watchlistsResponse.getWatchlists()) {
			...
			for (MarketsItem market : watchlistInstrumentsResponse.getMarkets()) {
				...
				if (market.getStreamingPricesAvailable() && market.getMarketStatus() == MarketStatus.TRADEABLE) {
					...
					tradeableCcys.add(tradeableEpic);
				}
			}
		}
	}
	...
	private void subscribeToLighstreamerPriceUpdates() throws Exception {
		tradeableCcys.retainAll(tradingCcys);
		Instant start = Instant.now();
		if (!tradeableCcys.isEmpty()){
			tradeableCcys.forEach((ccy) -> {
				try {
					LOG.info("Subscribing to Lightstreamer price updates for market: {} ", ccy);

					Runnable c = () -> {
						try{
							listeners.add(priceUpdateListener.listen(ccy));
						} catch (Exception e){

						}
					};

					listenerExecutor.submit(c);
					//listeners.add(mockPriceUpdateListener.listen(ccy));

					executorService.submit(priceUpdateHandler);


					//Instant finish = Instant.now();

					//LOG.info("Duration: " + Duration.between(start, finish).toMillis());
			...
			LOG.warn("None of configured trading ccys in the tradeable list.");
		}
      /*if (tradeableEpic != null) {
	...
	private void subscribeToLighstreamerChartUpdates() throws Exception {
		...
		if (tradeableEpic != null) {
			...
			listeners.add(streamingAPI.subscribeForChartCandles(tradeableEpic, "5MINUTE", new HandyTableListenerAdapter() {
				@Override
				public void onUpdate(int i, String s, UpdateInfo updateInfo) {
